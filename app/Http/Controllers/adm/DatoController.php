...
class DatoController extends Controller
{
    ...
    public function showEdit($seccion)
    {
        ...
        if($dato){
            ...
            if($seccion == 'terminos')
                $terminos  = json_decode($dato->terminos, true);
            if($seccion == 'mapa')
                $mapa  = $dato->mapa;
        }
        //los mandamos a la vista
        return view('adm.datos.edit', compact('telefono','email','direccion', 'terminos', 'mapa', 'seccion'));
    }
    ...
    public function update(Request $request)
    {
        ...
        if ($request->seccion=='telefono') {
            ...
            for ($i=0; $i < count($request->telefono); $i++) { 
                $telefono[$i]['tipo']    =  $request->tipo[$i];
                $telefono[$i]['seccion'] =  $request->seccion_tel[$i];
                $telefono[$i]['num']     =  $request->telefono[$i];
            }
        ...
        if($request->seccion == 'terminos'){
            ...
            foreach (LaravelLocalization::getSupportedLocales() as $key => $lang) {
                $terminos[$key]  = $request->terminos[$key];
            }
            $dato->terminos  = json_encode($terminos);
        }
        if($request->seccion == 'mapa'){
            $dato->mapa  = $request->mapa;
        }
        $dato->save();
